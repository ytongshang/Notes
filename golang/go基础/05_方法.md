...
- 如果结构体S，包含一个匿名字段T，那么这个结构体 S 就有了 T的方法。
...
- 一个ColoredPoint并不是一个Point，但它"has a"Point，**是组合而非继承的关系，并且它有从Point类里引入的Distance和ScaleBy方法。**内嵌字段会指导编译器去生成额外的包装方法来委托已经声明好的方法
