...
- 结构体的初始化，**无论哪种方法，如果不在定义结构体的包内部，都没法初始化未导出的值**
    - 按结构体变量定义的顺序指定**每个变量**的值，只在定义结构体的包内部使用，或者是在较小的结构体中使用
    - 通过成员名字和相应的值来初始化，未指定使用零值
...
- 编码相关的struct tag
    ...
    - tag中如果带有"omitempty"选项，那么如果该字段值为对应的零值，就不会输出到JSON串中
...
type Server struct {
    ...
    ServerIP   string `json:"serverIP,omitempty"` // 如果ServerIP为空的话，不序列化
